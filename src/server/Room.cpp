...
std::vector<std::string> Room::get_members() const
{
    ...
    {
        if (session) members.push_back(session->get_nick().name_);
    }
...
void Room::join(const std::shared_ptr<Session>& session)
{
    ...
    broadcast(format_info(session->get_nick().name_ + " joined " + room_name_));
    log_line("INFO", "room", session->get_nick().name_ + " joined room=" + room_name_);
}
...
void Room::leave(const std::shared_ptr<Session>& session)
{
    ...
    {
        broadcast(format_info(session->get_nick().name_ + " left " + room_name_));
        log_line("INFO", "room", session->get_nick().name_ + " left room=" + room_name_);
